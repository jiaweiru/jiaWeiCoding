...
class NeuralCoding(Pretrained):
    ...
    def __init__(self, modules=None, hparams=None, run_opts=None, freeze_params=True):
        ...
        self.header_struct = struct.Struct(
            "!"
            + str(len(self.stream_head))
            + "s"
            + str(len(self.identifier))
            + "s"
            + "I"
        )
    ...
    def compress(self, input_path: Path, output_path: Path):
        ...
        if not output_path.exists():
            logger.info(
                f"Output dir {output_path} does not exist, mkidr {output_path}."
            )
    ...
    def audio2comp(self, audio):
        with torch.no_grad():
            ...
            indices_list = self.mods.coder.encode(
                torch.nn.functional.pad(
                    audio,
                    (
                        0,
                        self.hparams.hop_length
                        - (audio.shape[1] % self.hparams.hop_length),
                    ),
                    "constant",
                )
            )
        ...
        write_header(
            fo, metadata, self.header_struct, self.stream_head, self.identifier
        )
    ...
    def comp2audio(self, compressed):
        ...
        with torch.no_grad():
            ...
            indices_list = torch.tensor(
                indices_list, dtype=torch.long, device=self.device
            ).unsqueeze(0)
...
if __name__ == "__main__":
    ...
    coder = NeuralCoding.from_hparams(
        source=source,
        hparams_file=filename,
        overrides=overrides,
        savedir=savedir,
        pymodule_file="",
    )  # set pymodule_file '' to avoid unnecessary symlink
    coder.run()
