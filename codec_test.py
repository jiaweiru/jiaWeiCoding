...
class CodecTest:
        ...
        if os.path.exists(config_path):
            ...
            recursive_update(
                config,
                {
                    "handlers": {
                        "file_handler": {
                            "filename": Path(self.hparams.exp_dir).joinpath("log.txt")
                        }
                    }
                },
            )
    ...
    def path_from_json(self):
        data = load_data_json(
            self.hparams.test_json, replacements={"data_root": self.hparams.data_folder}
        )
        data_ids = list(data.keys())
        path_list = [data[i]["path"] for i in data_ids]

        logger.info(
            f"There are a total of {len(data_ids)} audio files that require testing."
        )
    ...
    def audio_test(self):
        ...
        with tqdm(self.audio_list) as t:
            for path in t:
                ...
                encode_cmd = self.hparams.encode_sh.format_map(
                    {"input": path, "output": self.hparams.encode_dir}
                )
                decode_cmd = self.hparams.decode_sh.format_map(
                    {
                        "input": os.path.join(
                            self.hparams.encode_dir,
                ...
                encode_handle = subprocess.run(
                    encode_cmd,
                    shell=True,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                )
                decode_handle = subprocess.run(
                    decode_cmd,
                    shell=True,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                ...
                sf.write(
                    os.path.join(self.hparams.raw_dir, os.path.basename(path)), raw, sr
                )
                ...
                visqol_cmd = self.hparams.visqol_sh.format_map(
                    {"path": path, "decoded_path": decoded_path}
                )
                visqol_handle = subprocess.run(
                    visqol_cmd,
                    shell=True,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.STDOUT,
                ...
                visqol_score = float(
                    visqol_handle.stdout.decode("utf-8")
                    .split("\t")[-1]
                    .replace("\n", "")
                )
                visqol += visqol_score
                ...
                logger.debug(
                    f"File name:{os.path.basename(path)}, PESQ-WB:{pesq_score}, VISQOL:{visqol_score}"
                )
