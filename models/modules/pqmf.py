...
class PQMF(nn.Module):
    def __init__(self, N, M, trainable):
        super().__init__()
        self.N = N  # nsubband
        self.M = M  # nfilter
        try:
...
if __name__ == '__main__':
    ...
    # model params
    pqmf = PQMF(8, 64, trainable=True)
    audio = torch.randn(4, 1, 44100*2)
    audio_sub = pqmf.analysis(audio)
    audio_recon = pqmf.synthesis(audio_sub)
    snr = 10 * torch.log10((audio_sub ** 2).mean() / ((audio - audio_recon) ** 2).mean()).item()
    print(f'SNR of chirp_recomposed: {snr:.2f}dB')
    print(audio_sub.shape)

    flops, params = get_model_complexity_info(pqmf, (1, 3200), as_strings=True, print_per_layer_stat=True)
    print('Flops:  ' + flops)
    print('Params: ' + params)
    pytorch_total_params = sum(p.numel() for p in pqmf.parameters() if p.requires_grad)
