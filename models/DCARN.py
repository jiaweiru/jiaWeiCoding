...
from .modules import (VectorQuantizerEMA1D, 
                      ComplexConv2d, NaiveComplexBatchNorm2d, cPReLU, ComplexConvTranspose2d, NaiveComplexLSTM, NaiveComplexGRU, ComplexMHA, 
                      ConvSTFT, ConviSTFT)
        ...
        for idx in range(self.rnn_layers):
            rnn_list.append(
                self.rnn(
                    ...
                    batch_first=False,
                    projection_dim=self.input_size if idx == self.rnn_layers - 1 else None,
                )
            )
...
class ComplexAttention(nn.Module):
    ...
    def __init__(
        ...
        attn_list = []
        for idx in range(self.num_layers):
            attn_list.append(ComplexMHA(self.input_size, self.ff_size, self.n_heads, self.is_pe))
...
class DCARN(nn.Module):
    ...
    def __init__(
        ...
        self.conv_encoder = ComplexConvEncoder(self.kernel_size, self.kernel_stride, self.kernel_num, True)
        self.conv_decoder = ComplexConvDecoder(self.kernel_size, self.kernel_stride, self.kernel_num, True)
        ...
        self.rnn_encoder = ComplexRNN(rnn_input_size, self.rnn_layers, self.rnn_units, self.rnn_type)
        self.rnn_decoder = ComplexRNN(rnn_input_size, self.rnn_layers, self.rnn_units, self.rnn_type)

        self.attn_layers = attn_layers
        self.n_heads = n_heads
        attn_input_size = self.kernel_num[-1]
        self.attn_encoder = ComplexAttention(self.attn_layers, attn_input_size, attn_input_size * 4, self.n_heads, is_pe=True)
        self.attn_decoder = ComplexAttention(self.attn_layers, attn_input_size, attn_input_size * 4, self.n_heads, is_pe=True)
        # 4x in FFN
